...
if (isset($_SESSION['id_nv'])) {
    ...
    if ($totalPages > 1) {
        echo '<div></div>
                <div>';
        if ($currentPage > 1) {
            echo '<button class="navi"><a href="?act=quanlykhachhang&p=' . ($currentPage - 1) . '&size=' . $listsPerPage . '">&lt;</a></button>';
        }
        ...
        for ($i = $start; $i <= $end; $i++) {
            if ($i == $currentPage) {
                ...
                echo '<button class="navi"><a href="?act=quanlykhachhang&p=' . $i . '&size=' . $listsPerPage . '">' . $i . '</a></button>';
            }
        }
